// taken from simple-git
// https://github.com/steveukx/git-js
// modified to have merge functionality and other functions
...
(function() {
  ...
  */
  ...
  Git.prototype._suppressOutput = true;
  ...
  * @param {Function} [then]
  */
  Git.prototype.on = function(then) {
    this._suppressOutput = true;
    return this._run(['symbolic-ref --short -q HEAD'], function(err, data) {
      then && then(err, !err && data.trim());
    });
  };
  ...
  Git.prototype.status = function(then) {
    ...
    return this._run(['status --porcelain'], function(err, data) {
      if (!err) {
        ...
        if (commitStatus.changed) {
          // stuff has changed no need to check branch status
          then && then(err, commitStatus);
        } else {
          // no new changes, check if past changes committed
          this._run(['status --branch --porcelain'], function(err, data) {
            then && then(err, !err && this._parseBranchStatus(data));
          });
        }
      } else {
  ...
  */
  Git.prototype.clean = function(then) {
    this._suppressOutput = true;
    return this._run(['clean -fd'], function(err, data) {
      then && then(err);
    });
  };
  ...
  /**
  * Cleans (resets to latest commit) tracked files and directories
  */
  Git.prototype.reset = function(then) {
    this._suppressOutput = true;
    return this._run(['reset --hard'], function(err, data) {
      then && then(err);
    });
  };
  ...
  * @param {Function} [then]
  */
  Git.prototype.merge = function(message, branch, then) {
    this._suppressOutput = false;
    return this._run(['merge -m "%s" "%s"', message, branch], function(err, data) {
      then && then(err);
    });
  };
  ...
  * @param {Function} [then]
  */
  Git.prototype.pushUp = function(remote, branch, then) {
    this._suppressOutput = false;
    var command = "push";
    if (typeof remote === 'string' && typeof branch === 'string') {
      command = ['push -u "%s" "%s"', remote, branch];
    }
    if (typeof arguments[arguments.length - 1] === 'function') {
      then = arguments[arguments.length - 1];
    }
  ...
  */
  ...
  Git.prototype._parseBranchStatus = function(status) {
    // output when in sync:
    // ## site/spring.cat...origin/site/spring.cat
    // output when ahead:
    // ## site/spring.doge...origin/site/spring.doge [ahead 5]
  ...
  */
  ...
  Git.prototype._schedule = function() {
    if(!this._childProcess && this._runCache.length) {
      ...
      if (this._outputHandler && !this._suppressOutput) {
