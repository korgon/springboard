	...
	self.start = function(site) {
		...
		if (proxy_settings && proxy_settings.enable && proxy_settings.enable.value) {
			...
			if (catalog == 'smc') {
				...
				if (!site.catalogs[catalog]) return Promise.reject({ error: true, message: 'invalid site catalog' });

				let vars = site.catalogs[catalog].getVariables();

				settings.script.src = vars.COMPILED_LOADER;
				settings.script.path = site.directory + '/' + catalog + '/' + vars.MODULE_DIR;
			}

			console.log('precreation');
	...
	function attachIntercepts(settings) {
		...
		self.proxy.intercept({
			...
			if (settings.script.src) {
				resp.$('head').prepend('<script type="text/javascript" src="hooke/' + settings.script.src + '">');
			}
		...
		if (settings.script.src) {
			...
			self.proxy.intercept({
				...
				if(self.options.debug) logit.log('Hooke', 'serving script (' + settings.script.path + '/' + settings.script.src, 'green');
				return cycle.serve({
