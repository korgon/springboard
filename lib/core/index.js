...
* Write hooke (proxy with hoxy)
	+ proxy the proxy @ 1338 using browsersync
	+ use SMC templates or choose catalog (make dropdown)
...
function springboard() {
	...
	var bs = { app: undefined, proxy: undefined };

	// s3 connection
	var s3;

	// site and library repository objects
	var repos = { sites: undefined, library: undefined };
	...
	self.init = function(config) {
		...
		return userconf(options.user_config).then(userobj => {
			...
			return startBrowserSync(options.koa_port, options.app_port);
		}).then(bsinstance => {
			bs.app = bsinstance;
...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
	...
	function stopBrowserSync(bsinstance) {
		if (bsinstance) {
			bsinstance.exit();
		}
	}
