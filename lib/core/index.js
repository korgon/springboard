...
* Prevent 'module' folder from being autocreated until needed

* Implement playgrounds
* Fix site json files to contain relative paths for all
	+ thumb, default_url
	+ empty thumb should default to working.png
				...
				if (dependencies.length) {
					...
					for (let dependency of dependencies) {
						...
						if (site.catalogs[catalog_name].modules[parts[1]] && site.catalogs[catalog_name].modules[parts[1]].themes[parts[2]]) {
							...
							install_promises.push(library.catalogs[parts[0]].modules[parts[1]].install(info));
						}
					}
				}

				// module install
				install_promises.push(library.catalogs[info.catalog].modules[info.module].install(install_info));

				return Promise.all(install_promises);
			}).then(() => {
