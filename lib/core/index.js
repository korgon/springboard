...
avg ram usage 76MB, 79MB, 90MB, 100MB, 115MB
...
* Finish out Settings tabs
		...
		? default cloud files
	- Add history recording (could put off) for what purpose?
	...
	self.updateSite = (info) => {
		...
		if (info.name == site.name) {
			return site.update(info).then(() => {
				// do some things...
				if (info.proxy.enable.value && info.proxy.url.value) {
					return hooke.start(site);
