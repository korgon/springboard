...
avg ram usage 76MB, 79MB, 90MB, 100MB
...
let gitmo = require('_/gitmo');
let lib = require('./catalogs');

let fspro = require('_/fspro');

// local modules
let hooke = require('./hooke/');
let website = require('./site.js');
let userconf = require('./user.js');
let watchers = require('./watchers');
let browsersync = require('./bs.js');
let s3w = require('./s3w.js');
...
function springboard() {
	...
	self.updateUser = (userdata) => {
		try {
			return user.update(userdata).then(() => {
				...
				return gitmo.setConfig(options.sites_repo_dir, 'user.name', userdata.name);
			}).then(() => {
				return gitmo.setConfig(options.sites_repo_dir, 'user.email', userdata.email);
			});
		}
		catch(err) {
			return { error: true, message: err.message };
	...
	self.addSite = (details) => {
		...
		return repos.sites.status().then(stats => {
			...
			let commitmsg = 'CREATED >>> ' + site.name;
			return self.commitSite(commitmsg);
		}).then(() => {
			return self.pushSite();
		}).then(() => {
			let mergemsg = 'MERGED CREATION >>> ' + site.name;
	...
	self.editSite = (thesite, ignore) => {
		...
		if (sites[thesite]) {
			...
			return repos.sites.status().then(stats => {
				...
				return hooke.start(site, options.debug);
			}).then(() => {
				return site.getState();
			}).catch(err => {
	...
	self.installCatalog = (info) => {
		...
		if (library.catalogs[info.type]) {
			return library.catalogs[info.type].install(install_info).then(() => {
				...
				if (site_state.proxy.enable) {
					variables.proxy = {
						catalog: site_state.proxy.catalog.value,
						context: site_state.proxy.context.value,
					}
				}
...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
	...
	function mergeSite(message) {
		...
		return repos.sites.status().then(stats => {
			...
			return repos.sites.merge(site.branch, ['--allow-unrelated-histories', '--commit', '-m "' + message + '"']);
		}).catch(err => {
			if (err) {
				throw err;
			} else {
...
const capture = (url) => {
	...
	return new Promise(function(resolve, reject) {
		...
		let thumb_dir = self.directory + '/.thumbs/';
		let output_file = self.name + '.png';
		let output_path = thumb_dir + output_file;
		let url = 'http://localhost:' + global.port + '/sites/' + self.name + '/' + self.default_url;
