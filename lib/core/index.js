...
BUGS TODO
fix offline issues
color code all git log messages; similar to how proxy messages are color coded
follow symlinks in dirstruct
	...
	self.editSite = (thesite, ignore) => {
		...
		if (sites[thesite]) {
			...
			return repos.sites.status().then(stats => {
				...
				return repos.sites.clean(['-fdX --exclude=' + options.sites_repo_dir + '/' + thesite + '/*']);
			}).then(() => {
