	...
	self.installCatalog = (info) => {
		...
		if (library.catalogs[info.type]) {
			return library.catalogs[info.type].install(install_info).then(() => {
				...
				let variables = {
					siteid: site_data.catalogs[info.name].settings.siteid.value || site_data.settings.siteid.value || 'xxxxxx',
					query: site_data.catalogs[info.name].settings.query.value || 'q',
					site: site.name,
					catalog: info.name,
