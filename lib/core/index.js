...
BUGS TODO
somehow a file watcher continued watching the file afte switching sites
offline issues

NEXT TODO

* Implement playgrounds

* Look into double refresh on module install / disable / theme switch

* Better way to handle category and search mockups (context)?
...
function springboard() {
	...
	self.init = function(config) {
		...
		return userconf(options.user_config).then(userobj => {
			...
			logit.log('server started', 'http://localhost:' + options.app_port + '/', 'green');
	...
	self.updateUser = (userdata) => {
		...
		return user.update(userdata).then(() => {
			...
			return gitmo.setConfig(options.library_repo_dir, 'user.name', user.getName());
		}).then(() => {
			return gitmo.setConfig(options.library_repo_dir, 'user.email', user.getEmail());
		}).then(() => {
			if (createPlayground) {
				// first time setup should attempt to create playground
				if (options.debug) console.log('creating playground');
				...
				// not creating playground...
				return Promise.resolve();
			}
		}).then(() => {

		}).catch(err => {
			if (options.debug) console.log('Error in updateUser()', err);
			return { error: true, message: err.message };
		});
	}
	...
	self.getSites = () => {
		try {
			let arrayOfSites = Object.keys(sites).reduce(function(sites_array, site) {
			...
			playgrounds = playgrounds.sort((a, b) => {
				if (a.name < b.name) {
					return -1;
				} else if (a.name > b.name) {
					return 1;
			...
			arrayOfSites = playgrounds.concat(sortedSites);
	...
	self.reloadSite = () => {
		if (options.debug) console.log('SITE RELOAD REQUESTED!')
		if (!site) return { error: true, message: 'not editing a site...' };
	...
	self.fetchSites = (ignore) => {
		...
		return repos.sites.status().then(stats => {
			...
			if (stats.changes) {
				...
				throw { error: true, action: 'commit', location: 'loadSites', site: site ? site.name : null, message: 'There are uncommited changes!' };
	...
	self.loadSites = (ignore) => {
		...
		return repos.sites.status().then(stats => {
			...
			// load the sites
			logit.log('loading sites', 'from ' + options.sites_repo_dir, 'blue');
			...
			for (let stat of stats) {
				...
				if (stat.isDirectory() && !name.match(/^\./)) {
					site_promises.push(website(name, stat.path).then(website => {
						if (website && website.getData().type == 'site') {
			...
			// empty sites object (to start fresh)
			...
			for (let website of websites) {
				if (website.name) {
					...
					// only include 'site' types in the count
					if (website.getData().type == 'site') {
						count++;
					}
				}
			}
			...
			// logit.log('load complete', '', 'blue');
			console.log(' └ '.blue + loadmsg.blue);
	...
	self.addSite = (details) => {
		...
		return repos.sites.status().then(stats => {
			...
			let site_match = new RegExp(details.type + '/' + details.name);
			let found_template = false;

			branches.forEach(function(branch) {
				// look for new site branch
				if (branch.match(site_match)) {
					throw { error: true, action: 'addSite', message: details.type + '/' + details.name + ' branch already exists!' };
				}
			...
			// checkout new branch
			return repos.sites.checkout(details.type + '/' + details.name, ['-b']);
		}).then(() => {
			...
			// initialize created and creation
			return site.initializeCreation();
		}).then(() => {
			// update user config
			...
			// do not start editing if it is not a 'real' site
			if (details.type == 'site') {
			...
			if (options.debug) console.log('error adding site', err);
			throw err;
		});
	}
		...
		if (sites[thesite]) {
			return repos.sites.status().then(stats => {
				...
				branches.forEach(branch => {
					if (branch.match(sites[thesite].branch)) match = true;
				});

				if (!match) {
					throw { error: true, action: 'editSite', message: sites[thesite].branch + ' branch does not exists!' };
				}
				...
				if (user.getCurrentSite() != site.name) {
					if (options.debug) console.log('resetting site', site.name);
					return self.resetSite().then(() => {
						return repos.sites.clean(['-fd']);
					});
				} else {
					return Promise.resolve();
	...
	self.installCatalog = (info) => {
		...
		if (library.catalogs[info.type]) {
			return library.catalogs[info.type].install(install_info).then(() => {
				...
				return site.compile(info.name);
			}).then(() => {
				return watchers.watch(site);
			}).then(() => {
				return site.compile(info.name, 'sassModules');
			}).then(() => {
				return site.compile(info.name, 'js');
			}).then(() => {
	...
	self.compileCatalog = (info) => {
		...
		site.reload().then(() => {
			...
			if (site.catalogs[info.name]) {
				return site.compile(info.name).then(() => {
					return;
				}).catch(err => {
					throw { error: true, message: err.message };
				});
	...
	self.installModule = (catalog_name, info) => {
		...
		if (library.catalogs[info.catalog]) {
			...
			return watchers.stop().then(() => {
				...
				let install_promises = [];

				// gather dependencies from module and theme
				let dependencies = library.catalogs[info.catalog].modules[info.module].getData().requires || [];
				dependencies = dependencies.concat(library.catalogs[info.catalog].modules[info.module].themes[info.theme].getData().requires);

				// remove undefined requires
				dependencies = dependencies.filter((dependency) => { if (dependency != undefined) return dependency; })

				if (dependencies.length) {
					if (options.debug) console.log('installing dependencies...');
					for (let dependency of dependencies) {
						if (options.debug) console.log(dependency);
						let parts = dependency.split('/');
						let info = {
							name: parts[1],
							directory: install_info.directory,
							theme: parts[2]
						};

						// check if module installed
						if (site.catalogs[catalog_name].modules[parts[1]] && site.catalogs[catalog_name].modules[parts[1]].themes[parts[2]]) {
							// dependency is already installed, do nothing
						} else {
							// need to install dependency
							install_promises.push(() => {
								return library.catalogs[parts[0]].modules[parts[1]].install(info)
							});
						}
					}
				}
				...
				// synchronously install the module dependencies and the module
				return install_promises.reduce(function(install_promise, next) {
					return install_promise.then(next);
				}, Promise.resolve()).then(function() {
					// all things have installed
					return Promise.resolve();
				});
			}).then(() => {
				...
				return site.compile(catalog_name);
			}).then(() => {
	...
	self.resetSite = () => {
		if (!site) {
			return watchers.stop().then(() => {
				...
				return { error: false, site: null, action: 'reset', status: 'success' };
			}).catch(error => {
				throw { error: true, site: null, action: 'reset', status: 'failed', message: err.message };
			});
		} else {
			return watchers.stop().then(() => {
				return repos.sites.clean(['-fd']);
	...
	function repoInit(repo_dir, repo) {
		...
		return fspro.exists(repo_dir).then(stats => {
			if (stats) {
				...
				logit.log('initialization', 'clone remote repository');

				spinner = ora({
					text: 'cloning ' + repo_name + '...',
					color: 'red'
				}).start();
			...
			spinner && spinner.fail(' ' + repo_name + ' clone failure');
			console.log();
			cursor.hide();
	...
	function loadLastState() {
		...
		return fspro.exists(options.save_file).then(status => {
			...
			if (stats.branch == 'master') {
				// no current site
				throw 'load sites';
			} else if (stats.branch == 'site/' + current_site || stats.branch == 'playground/' + current_site) {
