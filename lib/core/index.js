...
* Finish out Settings tabs
	- Implement s3 capabilities incorporate into file viewer
		- add to cloud array on catalog creation
		- ignore and remove files that do not exist
...
function springboard() {
	...
	// constants
	const PLATFORM_OPTIONS = ['custom', 'magento', 'bigcommerce', 'miva', 'shopify', '3dcart', 'yahoo', 'volusion', 'commercev3', 'netsuite'];
		...
		if (library.catalogs[info.type]) {
			return library.catalogs[info.type].install(install_info).then(() => {
				...
				let variables = {
					...
					query: site_state.catalogs[info.name].settings.query.value || 'q',
					context: site_state.catalogs[info.name].settings.context.value || '',
					site: site.name,
					catalog: info.name,
					cdn: options.cdn_base_url
				}
	...
	self.compileCatalog = (info) => {
		...
		if (site.catalogs[info.name]) {
			...
			let variables = {
				siteid: site_state.catalogs[info.name].settings.siteid.value || site_state.settings.siteid.value || 'xxxxxx',
				query: site_state.catalogs[info.name].settings.query ? site_state.catalogs[info.name].settings.query.value : 'q',
				context: site_state.catalogs[info.name].settings.context.value || '',
				site: site.name,
				catalog: info.name,
				cdn: options.cdn_base_url
			...
			if (library.catalogs[info.catalog]) {
				...
				return watchers.stop().then(() => {
					...
					let variables = {
						...
						query: site_state.catalogs[catalog_name].settings.query.value || 'q',
