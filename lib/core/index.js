...
fix offline issues
when switching themes, no auto-refresh occurs
when enabling/disabling module no auto-refresh occurs
when modifying theme variables variable files don't get re-created (nor do they on 'force recompile')
^ boolean false values are not in the variable files
...
function springboard() {
	...
	self.loadSites = (ignore) => {
		...
		return repos.sites.status().then(stats => {
			...
			return repos.library.pull('origin', 'master');
		}).then(() => {
