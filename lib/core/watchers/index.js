				...
				if (site.catalogs[catalog]) {
					...
					this.compileInterval = setTimeout(function() {
						return site.catalogs[catalog].compile.templates().then(() => {
							...
							site.state = site.getState();

							let variables = {
								siteid: site.state.catalogs[catalog].settings.siteid.value || site.state.settings.siteid.value || 'xxxxxx',
								query: site.state.catalogs[catalog].settings.query ? site.state.catalogs[catalog].settings.query.value : 'q',
								site: site.name,
								catalog,
								cdn: options.cdn_base_url
							}

							return site.catalogs[catalog].compile.catalog(variables);
						}).then(() => {
