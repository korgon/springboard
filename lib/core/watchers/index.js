					...
					if (!source.module) {
						...
						return fspro.getJSON(file).then(contents => {
							...
							if (!fspro.compareObjects(last_state.catalogs[source.catalog].variables, contents.variables) || !fspro.compareObjects(last_state.catalogs[source.catalog].settings, contents.settings)) {
								...
								let variables = {
									siteid: contents.settings.siteid ? contents.settings.siteid.value : 'xxxxxx',
									context: contents.settings.context ? contents.settings.context.value : '',
									site: site.name,
								...
								compile_promises.push(site.catalogs[source.catalog].compile.loader(variables));
					...
					return fspro.getJSON(file).then(contents => {
						if (!fspro.compareObjects(last_state.proxy, contents.proxy) && contents.proxy.enable.value && site.catalogs[contents.proxy.catalog.value]) {
							...
							let variables = {
								...
								context: contents.settings.context ? contents.settings.context.value : '',
								site: site.name,
								catalog: contents.proxy.catalog.value,
								cdn: options.cdn_url + '/' + options.sites_base_dir,
								proxy: {
									catalog: contents.proxy.catalog.value ? contents.proxy.catalog.value : '',
									context: contents.proxy.context.value ? contents.proxy.context.value : ''
								}
							}

							return site.catalogs[contents.proxy.catalog.value].compile.loader(variables);
						} else {
	...
	function watchJs() {
		return new Promise(function(resolve, reject) {
			return fspro.exists(site.directory + '/js').then(status => {
				...
				if (status) watch_list.push(site.directory + '/js/**/*.js');
