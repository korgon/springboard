...
const user = (json_file, data) => {
	let state = {
		json_file,
		current_site: data.recent_sites[data.recent_sites.length - 1] || '',
		data
	}
...
const reload = (state) => ({
	reload: () => {
		return fspro.getJSON(state.json_file).then(contents => {
			if (contents && contents.s3) {
				state.data = contents;
				state.current_site = state.data.recent_sites[state.data.recent_sites.length - 1]
				return;
			} else {
...
const switchSite = (state) => ({
	switchSite: (site_name) => {
		let recents = state.data.recent_sites.indexOf(site_name);
		if (recents != -1) {
			state.data.recent_sites.splice(recents, 1);
		}

		if (state.data.recent_sites.push(site_name) > MAX_NUMBER_OF_RECENTS) {
			state.data.recent_sites.shift();
		}
