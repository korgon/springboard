...
const mod = (name, directory, catalog_directory, data) => {
	...
	return init(state).then(() => {
		...
		console.log('v3 mod init failed...');
	});
}
...
const loadTheme = (name, theme_dir, state) => {
	return thm(name, theme_dir).then(theme => {
		...
		if (err.code == 'ignore') {
			...
			err.message = 'v3 mod loadTheme(): ' + err.message;
			throw err;
		}
	});
}
	...
	install: (details) => {
		if (details && details.directory && details.name && details.theme) {
			...
			return fspro.exists(details.directory).then(exists => {
				...
				return fspro.exists(modules_dir);
			}).then(exists => {
				if (!exists) {
				...
				if (exists) {
					// if module folder already exists
				...
				// link the theme
				let theme_contents = state.themes[details.theme].getContents();
				return link(details.directory, install_dir, details.name, details.theme, theme_contents);
			}).then(() => {
...
const linkDirs = (state) => ({
	// link directories
	linkDirs: () => {
		// check enabled state
		if (state.data.enabled && state.data.theme && state.themes[state.data.theme]) {
			// ensure module is linked for compiling
			let theme_contents = state.themes[state.data.theme].getContents();

			return link(state.catalog_directory, state.directory, state.name, state.data.theme, theme_contents);
		} else {
			return Promise.resolve()
		}
	}
...
const link = (catalog_directory, module_directory, module_name, theme_name, theme_contents) => {
	// link locations
	let js_link = catalog_directory + '/' + variables.JS_DIR + '/' + module_name;
	let sass_link = catalog_directory + '/' + variables.SASS_DIR + '/' + module_name;
	let templates_link = catalog_directory + '/' + variables.TEMPLATES_DIR + '/' + module_name;
	...
	// source files
	let relative_path = '../' + variables.MODULE_DIR + '/';
	let real_js_link = relative_path + module_name + '/' + theme_name + '/' + variables.JS_DIR;
	let real_sass_link = relative_path + module_name + '/' + theme_name + '/' + variables.SASS_DIR;
	let real_templates_link = relative_path + module_name + '/' + theme_name + '/' + variables.TEMPLATES_DIR;
	...
	return unlink(catalog_directory, module_name).then(() => {
		...
		if (theme_contents.js) link_promises.push(fspro.linkUp(real_js_link, js_link));
		if (theme_contents.sass) link_promises.push(fspro.linkUp(real_sass_link, sass_link));
		if (theme_contents.templates) link_promises.push(fspro.linkUp(real_templates_link, templates_link));
				...
				return update_promise.then(() => {
					...
					if ((state.data.theme !== details.theme && details.enabled) || enable_flag) {
						...
						if (details.theme && state.themes[details.theme]) {
							let theme_contents = state.themes[state.data.theme].getContents();
							return link(state.catalog_directory, state.directory, state.name, details.theme, theme_contents);
						}	else {
