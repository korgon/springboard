...
const compileJs = (state) => {
	...
	return checkGeneratedDir(generated_dir).then(() => {
		return fspro.getFiles(js_dir).then(files => {
			console.log('CONCATJSDIR!!!!', js_dir);
			console.log(files);
			let read_promises = [];

			// sort files so that the concat has ._variables.js and module JS at top
			let variable_file_index = files.indexOf(js_variables_file);
			let variables_file = files.splice(variable_file_index, 1).pop();
			files = files.sort(function(a, b) { return a.split('/').length < b.split('/').length });
			files.unshift(variables_file);
			...
			files.forEach(file => {
				...
				if (file.match(regex_match)) read_promises.push(fspro.readFile(file));
			});
			return Promise.all(read_promises);
		}).then(contents => {
			return fspro.writeFile(js_concat, contents.join('\n\n'));
		});
	}).catch(err => {
