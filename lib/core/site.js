...
// initialization
const initializeCreation = (state) => ({
	initializeCreation: () => {
		return new Promise(function(resolve, reject) {
			// need to set any empty creator values to site creator
			// need to set any zero valued created values to site creation time
			// assuming an empty creator also means an empty created value
			let creator = state.data.creator;
			let created = state.data.created;

			let initialize_promises = [];

			// Add all things needing updates to the initialization promise array
			for (let cat in state.library.catalogs) {
				let catData = state.library.catalogs[cat].getData();

				if (!catData.creator) {
					catData.creator = creator;
					catData.created = created;
					initialize_promises.push(state.library.catalogs[cat].update(catData));

					for (let mod in state.library.catalogs[cat].modules) {
						let modData = state.library.catalogs[cat].modules[mod].getData();

						if (!modData.creator) {
							modData.creator = creator;
							modData.created = created;
							initialize_promises.push(state.library.catalogs[cat].modules[mod].update(modData));

							for (let thm in state.library.catalogs[cat].modules[mod].themes) {
								let thmData = state.library.catalogs[cat].modules[mod].themes[thm].getData();

								if (!thmData.creator) {
									thmData.creator = creator;
									thmData.created = created;
									initialize_promises.push(state.library.catalogs[cat].modules[mod].themes[thm].update(thmData));
								}
							}
						}
					}
			...
			if (initialize_promises.length) {
				console.log('updating to:', creator, '@', created);
				
				return Promise.all(initialize_promises).then(() => {
					return state.library.reloadCatalogs().then(resolve);
				})
			} else {
				return resolve()
			}
