...
function watchers() {
	...
	function watchJson() {
		...
		watcher.on('all', function(event, file) {
			...
			if (catalog) {
				let mod = sourceModule(file);
				if (!mod) {
					// change was to the catalog json
					console.log('catalog ' + catalog + ' changed!');
					return site.catalogs[catalog].compile.jsVariables().then(() => {
						console.log('compiled js variables for ' + catalog);
						return site.catalogs[catalog].compile.sassVariables();
					}).then(() => {
						console.log('compiled sass variables for ' + catalog);
					}).catch(err => {
						console.log(err);
					});
				} else {
					let theme = sourceTheme(file);
					if (!theme) {
						// change was to the module
						console.log('catalog ' + catalog + ' module ' + mod + ' changed!');
						return site.catalogs[catalog].compile.sassModules().then(() => {
							console.log('compiled sass modules for ' + mod);
						}).catch(err => {
							console.log(err);
						});
					} else {
						// change was to the module theme
						console.log('catalog ' + catalog + ' module ' + mod + ' theme ' + theme + ' changed!');
					}
				}
	...
	function watchTemplates() {
		...
		for (let catalog in catalog_vars) {
			watch_list.push(site.directory + '/' + catalog + '/' + catalog_vars[catalog].TEMPLATES_DIR + '/**/*.html');
			watch_list.push(site.directory + '/' + catalog + '/' + catalog_vars[catalog].TEMPLATES_DIR + '/**/*.htm');
		}
	...
	function watchCatalogJs() {
		...
		for (let catalog in catalog_vars) {
			watch_list.push(site.directory + '/' + catalog + '/' + catalog_vars[catalog].JS_DIR + '/**/*.js');
		}
