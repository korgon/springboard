...
const compile = (state) => ({
	compile: {
		all: () => {
			...
			compile_first_promises.push(compileSassModules(state));
			...
			return Promise.all(compile_first_promises).then(() => {
				...
				compile_second_promises.push(compileJs(state));
...
const compileSassModules = (state) => {
	...
	for (let mod in state.modules) {
		...
		console.log('compiling module sass!!!');
		console.log(mod);
...
const compileJsVariables = (state) => {
	...
	let js_variables_content = '/* springboard generated variables (do not edit manually) */\n';

	for (let avar in state.data.variables) {
		js_variables_content +=	'var ' + avar + ' = ';
