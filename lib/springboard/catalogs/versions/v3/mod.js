	...
	return loadThemes(state).loadThemes().then(() => {
		...
		if (state.data.enabled) {
			...
			if (state.data.theme && state.themes[state.data.theme]) {
				return setTheme(state).setTheme(state.data.theme);
			}
		} else {
			// ensure module is unlinked for compiling
	...
	update: (details) => {
		if (details) {
			if (fspro.compareKeys(state.data, details)) {
				...
				// if theme has changed or module re-enabled
				if (enable_flag || state.data.theme !== details.theme) {
					...
					if (state.data.theme && state.themes[state.data.theme]) {
						...
						update_promise = Promise.reject(new Error('Invalid theme selected.'));
					}
				}
