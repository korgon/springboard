...
const compileVariables = (state) => ({
	...
	compileVariables: () => {
		return compileJsVariables(state).then(() => {
			return compileSassVariables(state);
		}).catch(err => {
...
const compileSassVariables = (state) => {
	...
	for (let variable in state.data.variables) {
		let variables_data = state.data.variables[variable];

		if (variables_data.value && variables_data.value != '') {
			sass_variables_content +=	'$' + module_name + '_' + variable + ': ' + variables_data.value + ';\n';
		}
	}
...
const compileJsVariables = (state) => {
	...
	let js_variables_content = '// springboard generated variables for ' + module_name + '/' + state.name + '\n';
