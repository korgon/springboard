...
function springboard() {
	...
	self.commitSite = function(message, new_status) {
		...
		return repos.sites.status().then(stats => {
			// TODO check for changes first to see if commit is needed
			console.log('checking for changes before commiting...');
			console.log(stats);
			...
			return { error: false, site: site.name, action: 'commit', message: 'success', changes: new_status };
		}).catch(err => {
			logit.log('commit', site.name.bold + ' ' + err.message);
			return { error: true, site: site.name, action: 'commit', message: err.message };
		});
