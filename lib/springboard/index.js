...
--- ensure watchers are firing

* Add settings page and require initial setup (style it)
? Add shortcut keys...

* Rethink the v1 API
- site/update									// update site
- site/install								// install catalog
- site/vthree/update					// update catalog
- site/vthree/install					// install module
- site/vthree/slide/update		// update module (enable/disable/theme)
- site/vthree/slide/install		// install theme

* show files changed in commit modal (to prevent bad commits)

* Finish out UI tabs
Add module install modal (using markdown prefered)
Finish UI design for inputs
Add json watcher (for compiling amoung other things)
^ compile variables and module files (._variables.js, ._modules.scss, ._variables.scss)
...
function springboard() {
	...
	self.installModule = function(info) {
		...
		if (site && site.name) {
			let install_info = {
				name: info.name,
				directory: site.catalogs[info.catalog].directory,
				theme: 'default'
			}
			if (library.catalogs[info.type]) {
				return library.catalogs[info.type].install(install_info).then(() => {
	...
	// disable module
	self.updateModule = function(info) {
		// TODO
		// use module.update()
		console.log(info);
	}
	...
	self.installTheme = function(info) {
		if (site && site.name) {
			let site_details = site.getSite();

			if (!site_details.catalogs[info.catalog]) {
				// invalid site catalog
				return Promise.reject({ error: true, message: 'catalog does not exist in site' });
			}
	...
	// set theme
	self.setTheme = function(info) {
		//return site.catalogs.vthree.modules.slide.setTheme('blocky');
		if (site && site.name) {

			if (!site.catalogs[info.catalog]) {
				// invalid site catalog
				return Promise.reject({ error: true, message: 'catalog does not exist in site' });
			}

			if(!site.catalogs[info.catalog].modules[info.module]) {
				// invalid site module
				return Promise.reject({ error: true, message: 'module does not exist in site' });
			}

			// install the theme
			return site.catalogs[info.catalog].modules[info.module].setTheme(info.theme).then(() => {
				return site.getSite();
			}).catch(err => {
				throw { error: true, message: err.message };
			});
...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
	...
	function mergeSite(message) {
		...
		return repos.sites.status().then(stats => {
			...
			logit.log('merged', site.name + ' >>> ' + 'master', 'pass');
			return;
		}).catch((err) => {
