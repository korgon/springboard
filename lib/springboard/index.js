...
* Finish out UI tabs
Add module install modal (using markdown readme (for description) prefered)

Finish UI design for inputs
Add json watcher (for compiling amoung other things)
^ compile variables and module files (._variables.js, ._modules.scss, ._variables.scss, theme/._varaibles.scss);
...
function springboard() {
	...
	self.reloadSite = function() {
		if (!site) return { error: true, message: 'not editing a site...' };

		return site.reload().then(() => {
			watchers.watch(site);
	...
	self.installCatalog = function(info) {
		...
		if (library.catalogs[info.type]) {
			return library.catalogs[info.type].install(install_info).then(() => {
				...
				// compile new catalog after watchers established (kinda hacky with the setTimeout but... meh)
				setTimeout(site.catalogs[info.name].compile.all, 350);	// waiting bout tree-fiddy
	...
	// update catalog
	self.updateCatalog = function(catalog_name, info) {
		if (!site) return Promise.reject({ error: true, message: 'not editing a site...' });
	...
	self.installModule = function(info) {
		...
		if (site && site.name) {
			...
			if (library.catalogs[info.type]) {
				...
				// invalid site catalog
				return Promise.reject({ error: true, message: 'failed to installModule: catalog does not exist in site' });
			}
		} else {
	...
	self.updateModule = function(catalog_name, module_name, info) {
		if (!site) return Promise.reject({ error: true, message: 'not editing a site...' });
	...
	// update module theme
	self.updateModuleTheme = function(catalog_name, module_name, theme_name, info) {
		if (!site) return Promise.reject({ error: true, message: 'not editing a site...' });
