	...
	function repoInit() {
		...
		return new Promise(function(resolve, reject) {
			try {
				...
				if (!(fs.existsSync(options.sites_repo_dir))) {
					logit.log('initialization', 'cloning site repository');
					git.clone(options.sites_repo, options.sites_repo_dir, function (err) {
						if (err) return reject(err);
					}).then(function() {
						git._baseDir = options.sites_repo_dir;
						return resolve(true);
					});
				} else {
					git._baseDir = options.sites_repo_dir;
				...
				// if (!(fs.existsSync(options.modules_repo_dir))) {
				// 	logit.log('initialization', 'cloning module repository');
				// 	gitmod.clone(options.modules_repo, options.sites_repo_dir, function (err) {
				// 		if (err) return reject(err);
				// 	}).then(function() {
				// 		gitmod._baseDir = options.modules_repo_dir;
				// 		return resolve(true);
				// 	});
				// } else {
				// 	gitmod._baseDir = options.modules_repo_dir;
		...
		return new Promise(function(resolve, reject) {
			...
			git.on(function(err, data) {
				...
				if (data == 'master') {
					...
					if (user.current_site && user.current_site.toLowerCase() != null) {
						// on current site branch
						if (data.indexOf(user.current_site) > -1) {
							try {
								...
								self.editSite(user.current_site).then(function() {
									return resolve(true);
								}).catch(function(err) {
									return reject(err);
								})
