...
const DEFAULT_TEMPLATE = '<script type="ss/template" target=""></script>';
...
var loadModules = (state) => ({
    ...
    try {
      var folders = fs.readdirSync(state.directory + '/' + variables.MODULE_DIR);
    }
    catch(err) {
      console.error(err);
    }
    ...
    for (let folder of folders) {
      ...
      let module_dir = state.directory + '/' + variables.MODULE_DIR + '/' + folder;
