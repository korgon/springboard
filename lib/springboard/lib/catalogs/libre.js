...
lib.load('/home/korgon/Work/springboard/springboard-library').then(l => {
	...
	return library.catalogs.v3.install({ name: 'v3', directory: dir });
}).then(() => {
	return lib.load(dir);
}).then(l => {
	testsite = l;
	...
	return library.catalogs.v3.modules.slideout.install({ name: 'slideout', directory: testsite.catalogs.v3.directory, theme: 'default' });
}).then(() => {
	...
	return library.catalogs.v3.modules.slideout.themes.blocky.install({ directory: testsite.catalogs.v3.modules.slideout.directory });
}).then(() => {
...
// Next TODO
// make entire module PROMISIFIED ?  YES!!!
