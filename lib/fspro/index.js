...
* @return {promise}
*/
var concatDir = function(directory, output) {
...
var getDirStruct = function(directory, structure) {
	console.log('DIRSTRUCT', directory);
	...
	return lstat(directory).then(stat => {
		if (stat && stat.isDirectory()) {
			if (!structure) {
				console.log('CREATING ROOT STRUCT');
				...
				for (let sub of relative_path.split('/')) {
					...
					if (structure_location.contents[sub]) {
						...
						structure_location.contents[sub] = { type: 'directory', path: relative_path, contents: {} };
						structure_location = structure_location.contents[sub];
					}
			...
			console.log('readingdir....', directory)
		...
		file_stats.forEach(function(stats) {
			...
			if (stats.isSymbolicLink()) {
				...
				console.log('RECURSIVE CHECK ON', stats.path);
			} else {
				// regular file
				let file_name = path.basename(stats.path.replace(directory, ''));
				let file_path = stats.path.replace(structure.root.path, '');
				console.log('regular file found', file_name);
				current_struct.contents[file_name] = { type: 'file', path: file_path, extension: path.extname(file_name) };
				console.log(current_struct);
