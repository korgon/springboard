...
function EditorCtrl($scope, $location, $window, $q, focus, usermanager, sitemanager, modalmanager) {
	...
	sitemanager.reloadSite().then(function(site) {
		vm.site = site;
		vm.root_url = $window.location.origin + '/sites/' + vm.site.name + '/';
		...
		var default_url = vm.root_url + vm.site.default_html;
	...
	vm.toggleCloudFile = function(path) {
		...
		console.log(vm.site.cloud_files);
		vm.checkChange();
	}
	...
	vm.getScriptTag = function(catalog_type) {
		if (catalog_type == 'v3') {
			return '<!-- Springboard Catalog Init -->\r<script type="text/javascript">document.write(unescape(\'%3Cscript src="\' + \'//\' + window.location.host + window.location.pathname.substr(0, window.location.pathname.lastIndexOf("/") + 1) + \'generated/catalog.js"%3E%3C/script%3E\'));</script>';
		}
	}
