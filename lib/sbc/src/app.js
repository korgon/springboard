...
function gallery(ignore) {
	return {
		...
		controllerAs: 'vm',
		resolve: {
			checkSite: function($q, $location, $route, sitemanager, usermanager) {
				...
				return usermanager.status().then(function(status) {
					...
					if (err) {
						if (err.error && err.action == 'commit' && err.site) {
							return $q.reject('uncommitted');
						} else if (err.error && err.action == 'commit' && !err.site) {
							return $q.reject('mastercontamination');
