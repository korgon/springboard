...
function springboard() {
	...
	*/
	...
	var options = {
		...
		proxy: "localhost",
		proxy_port: 1338,
		proxy_dir: "/searchspring/springboard/"
	}
	...
	self.loadSites = function() {
		...
		for (var folder of folders) {
			...
			try {
				...
				sites[folder] = new website(dir_sites + folder + '/');
				...
				console.error('failed to loadsites: ', err);
			}
		}
		if (folders)
		...
		return new Promise(function(resolve, reject) {
			...
			if (sites[usethis]) {
				...
				yield new Promise(function(resolve, reject) {
					try {
						git.checkout('site/' + checkoutsite.name, {cwd: options.mockup_dir}, function (err) {
							if (err) throw err;
							return resolve(true);
						});
					}
	...
	*/
	...
	function writeConfig() {
		try {
			...
			console.error('failed to writeconfig: ', err);
		}
		return;
	}
		...
		return new Promise(function(resolve, reject) {
			try {
				browserSync({
					...
					port: options.proxy_port,			// port number
					...
					var proxy_url = 'http://' + options.proxy + ':' + options.proxy_port + options.proxy_dir;
					console.log(proxy_url.blue);
					return resolve(true);
				});
			}
			catch(err) {
