	...
	self.init = function() {
		...
		return new Promise(function(resolve, reject) {
			...
			repoInit().then(function(reposites) {
				return startBrowserSync().then(function() {
					if (Object.keys(reposites).length > 0) {
						// start watching a site
						return self.watchSite().then(function() {
							return resolve(true);
						}).catch(function(err) {
						...
						fs.copy(site_dir + 'templates/scss/' + details.template, site_folder + '/scss', function(err) {
							...
							fs.copy(site_dir + 'templates/js/', site_folder + '/js', function(err) {
								...
								.then(function() {
									...
									return reject('Failed to push files to the repo...', err);
								}).then(function() {
									console.log('about to start watching new site');
									...
									return reject('Failed to commit files to the repo...', err);
								});
							});
						});
					});
