...
// manages github and s3 connection for sites
...
* modules for ajax_catalog, autocomplete and pr_widget
* future modules (angular_catalog)

things to do:
================================================================================
1.	finish modularizing
		|-	allow for multiple modules per site (mockup type)
		|-	allow for modules of modules [extensions] (ex: slideout)
...
function springboard() {
	...
	var sites_dir = global.site_repository_dirname + '/sites';
		...
		return new Promise(function(resolve, reject) {
			...
			self.commitSite().then(function() {
				git.checkout('master', function(err) {
					...
					if (err) {
						logit('offline', 'Cannot pull from remote repository', 'fail');
					}
					return true;
				}).then(function() {
					...
					if (options.current_site != 'none') self.useSite(options.current_site);
					// else {
	...
	self.newSite = function(details) {
		...
		return new Promise(function(resolve, reject) {
			...
			if (details === undefined || details.name === undefined) {
				...
				self.commitSite().then(function() {
					// create branch from site/_template
					git.checkout('master', function(err) {
						...
						// TBD move this to a function
						// createHTML(type)  // standard | mobile | category
						var scripts = '<html>\n\t<head></head>\n\t<body>';
						...
						self.mergeSite(commitmsg).then(function() {
	...
	self.commitSite = function(message, status) {
		...
		return new Promise(function(resolve, reject) {
			...
			if (site.siteid === undefined) {
				...
				site.setStatus({ status: undefined, gitstatus: status });
			...
			git.add([sites_dir + '/' + tempsite.name, sites_dir + '/.thumbs'], function(err, data) {
				...
				if (err) {
					...
					logit('commit', tempsite.name.bold + ' no changes to commit');
			...
			self.commitSite(message, 'pushed').then(function() {
				if (site.status == 'new') {
					git.pushUp('origin', 'site/' + site.name, function(err) {
						if (err) return reject(err);
						site.setStatus({ status: undefined, gitstatus: 'pushed' });
	...
	self.mergeSite = function(message) {
		...
		return new Promise(function(resolve, reject) {
			...
			stopWatch();
			self.commitSite(message, 'merged').then(function(){
				git.checkout('master', function(err) {
					if (err) return reject(err);
					...
					site.setStatus({ status: undefined, gitstatus: 'merged' });
	...
	self.publishSiteMockup = function() {
		...
		return new Promise(function(resolve, reject) {
			...
			site.capture().then(function() {
				...
				logit('publish', site.name + ' has been published to S3 MOCKUP', 'pass');
				return resolve( { site: site.name, action: 'publish', status: 'success' } );
			}).catch(function(err) {
	...
	self.publishSiteLive = function() {
		...
		return new Promise(function(resolve, reject) {
			...
			site.capture().then(function() {
				...
				logit('publish', site.name + ' has been published to S3 LIVE', 'pass');
				return resolve( { site: site.name, action: 'publish', status: 'success' } );
			}).catch(function(err) {
				logit('publish', 'failed to publish' + site.name, 'fail');
				console.error(err.red);
	...
	function loadModules() {
		...
		return new Promise(function(resolve, reject) {
			...
			for (var folder of folders) {
				...
				try {
					modules[folder] = new mod({ id: folder, directory: module_dir });
			...
			for (var a_mod in modules) {
				...
				line += modules[a_mod].name + ' v' + modules[a_mod].version;
	...
	function stopWatch() {
		// blinding the eyes
		if (eye_of_sauron) eye_of_sauron.end();
		if (eye_of_horus)	eye_of_horus.end();
	...
	function watchScss() {
		...
		gulp.task('css', ['scss'], function() {
			...
			.pipe(gulp.dest(site.directory + '/css/'))
			.pipe(gcb(function() {
				site.setStatus({ status: undefined, gitstatus: 'uncommited' });
