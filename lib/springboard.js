...
function springboard() {
			...
			if (sites[thesite]) {
				...
				git.checkout(site.branch, function(err) {
					if (err) {
						...
						return reject({ error: true, site: self.name, action: 'checkout', status: 'failed', message: err });
					}
				}).clean().then(function() {
		...
		return new Promise(function(resolve, reject) {
			git.status(function(err, data) {
				if (err) {
					...
					return reject({ error: true, site: site.name, message: err.message });
				}
				// check for file changes
				if (data.changed) {
					return resolve({ error: false, site: site.name, message: 'files were changed', changes: data });
				} else if(data.ahead) {
					return resolve({ error: false, site: site.name, message: data.count + ' commits to be pushed' });
				} else {
					// no changes
					return resolve({ error: false, site: site.name, message: 'no changes to commit' });
				}
			});
		});
	}
			...
			self.commitSite(message, 'pushed').then(function() {
				...
				if (site.status == 'new') {
					git.pushUp('origin', 'site/' + site.name, function(err) {
						if (err) return reject({ error: true, site: site.name, action: 'push', message: err.message });
			...
			git.on(function(err, data) {
				...
				if (data == 'master') {
					...
					if (data.indexOf(options.current_site) > -1) {
						...
						site = new website({ name: folder, directory: sites_dir + '/' + options.current_site }, options.user);
					} else {
	...
	function gitReset() {
		// promisified
		return new Promise(function(resolve, reject) {
			git.clear().reset().then(function(err, data) {
				if (err) return reject(err);
				return resolve(true);
			});
		});
