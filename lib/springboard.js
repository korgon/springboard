...
function springboard() {
	...
	*/
	...
	var options = {
		...
		sites_repo: "git@bitbucket.org:searchspring/searchspring-sites.git",
		current_site: "none"
	}
				...
				git.checkout('site/' + details.name, {cwd: site_dir, args: '-b'}, function (err) {
					...
					fs.copy(site_dir + 'templates/scss/' + details.template, site_folder + '/scss', function(err) {
						...
						fs.copy(site_dir + 'templates/js/', site_folder + '/js', function(err) {
							...
							// push new branch
							var commitmsg = options.user.name + '@springboard >>> ' + site.name + ' js files';
							site.commit(commitmsg)
							.then(function() {
								return site.push()
							}).catch(function() {
								...
								logit('new site', 'new branch created for ' + site.name + ' @ ' + site.folder, 'pass');
								return resolve(site);
							}).catch(function() {
	...
	self.publishSite = function(usethis) {
		...
		return new Promise(function(resolve, reject) {
			...
			if (sites[usethis]) {
				...
				thesite.checkout()
				.then(function() {
					var publishmsg = options.user.name + '@springboard >>> ' + thesite.name + ' published';
					...
					.then(function() {
						...
						logit('publish', 'failed to capture image for ' + thesite.name, 'fail');
						console.log(err)
	...
	*/
	...
	function watchSass() {
		...
		gulp.task('sass', function() {
			...
			.pipe(gcb(function() {
				var commitmsg = options.user.name + '@springboard >>> ' + site.name + ' scss/css files';
	...
	function watchHtml() {
		...
		gulp.task('html', function() {
			...
			.pipe(gcb(function() {
				var commitmsg = options.user.name + '@springboard >>> ' + site.name + ' html files';
				site.commit(commitmsg).then(function() {
					logit('commit', commitmsg, 'pass');
					reload();
				}).catch(function(err) {
