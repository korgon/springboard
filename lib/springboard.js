	...
	*/
	...
	function loadModules() {
		...
		return new Promise(function(resolve, reject) {
			...
			try {
				var folders = fs.readdirSync(dir_modules);
			}
			catch(err) {
				console.error(err);
				return reject(err);
			...
			for (var folder of folders) {
				// ignore non directories... or hidden folders (^.*)
				if (!fs.lstatSync(dir_modules + folder).isDirectory() || folder.match(/^\./)) continue;
				// add to module object array for versions
				modules[folder] = [];
				// read version directories
				try {
					var versions = fs.readdirSync(dir_modules + folder);
				}
				catch(err) {
					console.error(err);
					return reject(err);
				...
				for (var version of versions) {
					var dir_version = dir_modules + folder + '/' + version;
					if (!fs.lstatSync(dir_version).isDirectory() || version.match(/^\./)) continue;
					try {
						// create object for module version
						modules[folder][version] = new mod({ name: folder, version: version, directory: dir_version });
