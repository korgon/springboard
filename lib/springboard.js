	...
	self.newSite = function(details) {
		...
		return new Promise(function(resolve, reject) {
			if (details.name === undefined) {
				...
				if (fs.existsSync(options.mockup_dir + 'sites/' + details.name)) {
					return reject(Error('cannot create site: site exists.'));
				}
				// create new branch and switch to it
				git.checkout('site/' + details.name, {cwd: options.mockup_dir, args: '-b'}, function (err) {
					if (err) {
						return reject(err);
					}
					// create new folder
					var site_folder = options.mockup_dir + 'sites/' + details.name;

					// copy over templates
					fs.copy(options.mockup_dir + 'templates/scss/' + details.template, site_folder + '/scss', function(err) {
						if (err)
							return reject('failed to copy scss templates: ' + err);

						fs.copy(options.mockup_dir + 'templates/js/config.js', site_folder + '/js/' + details.name + '.js', function(err) {
							if (err)
	...
	self.useSite = function(usethis) {
		...
		return new Promise(function(resolve, reject) {
			...
			if (sites[usethis]) {
				...
				repoPull(site.name).then(function() {
					console.log('using '.yellow + site.name.yellow);
					//checkout git branch
					try {
						git.exec({args: 'checkout site/' + site.name, cwd: options.mockup_dir}, function (err) {
							if (err) {
								return reject(err);
							}
	...
	*/
		...
		return new Promise(function(resolve, reject) {
			try {
				if (!(fs.existsSync(options.mockup_dir))) {
					...
					repoPull().then(function() {
						self.loadSites();
						return resolve(true);
					}, function(err) {
						self.loadSites();
						return reject('failed to pull repository', err);
					});
				}
			}
			catch(err) {
	...
	function repoPull(branch) {
		...
		return new Promise(function(resolve, reject) {
			// pulls down branch
			try {
				git.pull('origin', branch, {cwd: options.mockup_dir}, function (err) {
					if (err) {
						return reject(err);
					}
					return resolve(true);
	...
	function repoAdd() {
		...
		return new Promise(function(resolve, reject) {
			try {
				git.exec({args: 'add sites/' + site.name, cwd: options.mockup_dir}, function (err) {
					if (err) {
						return reject(err);
					}
					return resolve(true);
	...
	function repoCommit(message) {
		...
		return new Promise(function(resolve, reject) {
			try {
				git.exec({args: 'commit -m "' + message + '"', cwd: options.mockup_dir}, function (err) {
					if (err) {
						return reject(err);
					}
					return resolve(true);
	...
	function repoPush() {
		...
		return new Promise(function(resolve, reject) {
			try {
				git.push('origin', 'site/' + site.name, {cwd: options.mockup_dir}, function (err) {
					if (err) {
						return reject(err);
					}
					return resolve(true);
	...
	function repoMerge() {
		...
		// 		git.merge('site/' + site.name, {cwd: options.mockup_dir}, function (err) {
		// 			if (err) {
		// 				return reject(err);
		// 			}
		// 			return resolve(true);
