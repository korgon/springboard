				...
				self.commitSite().then(function() {
					...
					git.checkout('CORE', function(err) {
						...
						if (details.template != 'none') {
							fs.copySync(site_dir + 'templates/scss/' + details.template, site_folder + '/scss', function(err) {
						...
						fs.copySync(site_dir + 'templates/js/', site_folder + '/js', function(err) {
	...
	self.publishSite = function() {
		...
		return new Promise(function(resolve, reject) {
			site.capture().then(function() {
				...
				// TBD some s3 stuff...
				logit('publish', site.name + ' published to the cdn', 'warn');
				return resolve( { site: site.name, action: 'publish', status: 'success' } );
			}).catch(function(err) {
				console.error(err);
				return reject(err);
