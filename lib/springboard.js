	...
	self.loadSites = function() {
		...
		return new Promise(function(resolve, reject) {
			...
			self.commitSite().then(function() {
				git.fetch('master', function (err) {
					if (err) return reject(err);
				}).checkout('master', function(err) {
					if (err) return reject(err);
				}).then(function() {
					// copy thumbs into cache
					fs.copySync(site_dir + 'sites/.thumbs', '.cache/sites/thumbs', {recursive: true}, function(err) {
						if (err) return reject(err);
					});
					// load the sites
					var total = 0;
			...
			if (details === undefined || details.name === undefined) {
				...
				self.commitSite().then(function() {
					...
					git.checkout('CORE', function(err) {
						...
						// create new folder
						site_folder = site_dir + 'sites/' + details.name;
						fs.mkdirSync(site_folder);
						// copy over templates
						if (details.template != 'none') {
							fs.copySync(site_dir + 'templates/scss/' + details.template, site_folder + '/scss', {recursive: true}, function(err) {
						...
						console.log('finished with scss...');

						fs.copySync(site_dir + 'templates/js', site_folder + '/js', {recursive: true}, function(err) {
							if (err) return reject('failed to copy js templates: ' + err);
						});
						console.log('finished with js...');
						...
						console.log('prior to site object creation...');
		...
		return new Promise(function(resolve, reject) {
			...
			git.add(['sites/' + site.name, 'sites/.thumbs'], function(err, data) {
				...
				if (err) {
					...
					logit('commit', site.name.bold + ' no changes to commit', 'warn');
		...
		return new Promise(function(resolve, reject) {
			...
			git.checkout('master', function(err) {
				...
				self.watchSite().then(function() {
					logit('merged', site.name + ' >>> ' + 'master', 'blue');
		...
		return new Promise(function(resolve, reject) {
			try {
				...
				if (!(fs.existsSync(site_dir))) {
					...
					git.clone(options.sites_repo, site_dir, function (err) {
						if (err) return reject(err);
					}).then(function() {
						git._baseDir = site_dir;
						return resolve(true);
					});
				} else {
					logit('syncing repository', 'updating local copy of repository');
					git._baseDir = site_dir;
					if (options.current_site && options.current_site != 'none') {
						// checkout current site to prepare to commit it in siteload
						git.checkout('site/' + options.current_site, function(err) {
							if (err) return reject(err);
						}).then(function() {
							return resolve(true);
						});
