		...
		return new Promise(function(resolve, reject) {
			...
			self.commitSite().then(function() {
				git.checkout('master', function(err) {
					...
					if (err) {
						logit.log('offline', 'Cannot pull from remote repository', 'fail');
					}
					return true;
				}).then(function() {
				...
				logit.log('loading sites', 'failed to commit site first', 'fail');
				return reject(err);
			});
		});
	}
	...
	function watchScss() {
		...
		gulp.task('scss', function() {
	        ...
	        sourceMap: 'scss'
				...
				console.log(err);
				logit.log('scss', 'failed to compile', 'fail');
