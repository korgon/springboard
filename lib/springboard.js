...
function springboard() {
	...
	*/
					...
					git.checkout('site/' + details.name, {cwd: site_dir, args: '-b'}, function (err) {
						...
						fs.copy(site_dir + 'templates/scss/' + details.template, site_folder + '/scss', function(err) {
							...
							fs.copy(site_dir + 'templates/js/', site_folder + '/js', function(err) {
								...
								console.log(commitmsg.green)
								site.commit(commitmsg).then(function() {
									git.push('origin', 'site/' + site.name, {args: " -u", cwd: site_dir}, function (err) {
	...
	self.commitSite = function(usethis) {
		console.log(usethis);
		// promisified
		return new Promise(function(resolve, reject) {
	...
	*/
	...
	function repoInit() {
		...
		return new Promise(function(resolve, reject) {
			try {
				repo = git();
				repo.outputHandler(function (command, stdout, stderr) {
					stdout.pipe(process.stdout);
					stderr.pipe(process.stderr);
				});
				if (!(fs.existsSync(site_dir))) {
					logit('initialization', 'cloning repository');
					repo.clone(options.sites_repo, site_dir, function (err) {
						if (err) return reject(err);
					}).then(function() {
						repo._baseDir = site_dir;
					});
				} else {
