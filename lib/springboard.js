...
function springboard() {
		...
		return new Promise(function(resolve, reject) {
			...
			git.checkout('master', function(err) {
				...
				if(sites[options.current_site]) {
					self.editSite(options.current_site);
				}
	...
	self.commitSite = function(message, status) {
		...
		return new Promise(function(resolve, reject) {
			...
			git.status(function(err, changes) {
				...
				if (changes.changed) {
					...
					git.add([sites_dir + '/' + site.name], function(err, data) {
						...
						return resolve({ error: false, site: site.name, action: 'commit', message: 'success', changes: changes });
					});
				} else {
					// no changes
	...
	function gitResest() {
		...
		return new Promise(function(resolve, reject) {
			git.clear().reset().then(function(err, data) {
				logit.log('reset', 'current git branch reset');
			});
		});
	}
