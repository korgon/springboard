...
module.exports = function(springboard) {
	...
	return {
		...
		putUser: function*() {
			...
			userdata.user.email = userdata.user.email.toLowerCase().trim();
			userdata.user.name = userdata.user.name.toLowerCase().trim();
			userdata.user.s3.key_id = userdata.user.s3.key_id.trim();
			userdata.user.s3.key_secret = userdata.user.s3.key_secret.trim();

			// check if username is valid
			if (!userdata.user.name.match(/^[\w]{3,}$/)) {
			...
			if (!userdata.user.email.match(/[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?/)) {
				this.response.status = 400;
			...
			if (userdata.user.s3.key_id && !userdata.user.s3.key_id.match(/^\w{20}$/)) {
				this.response.status = 400;
			...
			// check if s3 id is valid
			if (userdata.user.s3.key_secret && !userdata.user.s3.key_secret.match(/^[\W|\w]{40}$/)) {
				this.response.status = 400;
				this.response.body = { error: true, message: 'invalid s3 secret' };
				return;
			}

			this.response.body = yield springboard.updateUser(userdata);
		},
		...
		// fetch and load sites
		fetchSites: function*() {
			try {
				let data = yield springboard.fetchSites();
				this.response.type = 'json';
		...
		publishFile: function*() {
			...
			try {
				this.response.body = yield springboard.publishFile(info.path);
			}
			catch(err) {
				this.response.type = 'json';
				this.response.body = { error: true, message: err.message || err, action: err.action || 'unknown' };
