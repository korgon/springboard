...
module.exports = function(springboard) {
	return {
		...
		updateTheme: function*() {
			...
			// going to trust that this data is sanitary for now...
			let info = this.request.body.fields;
			let catalog = this.params.catalog;
			let mod = this.params.module;
			let theme = this.params.theme;

			if (catalog && mod && theme) {
				//this.response.body = springboard.updateCatalog(catalog, info);
				this.response.body = { error: true, message: catalog + ' update module ' + mod + ' theme ' + theme };
				return;
			} else {
				this.response.body = { error: true, message: 'Catalog, module and theme are required!' };
				return;
			}
		}
