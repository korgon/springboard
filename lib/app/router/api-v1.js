...
module.exports = function(springboard) {
	...
	return {
		...
		putUser: function*() {
			...
			userdata.user.email = userdata.user.email.toLowerCase();
			userdata.user.name = userdata.user.name.toLowerCase();
			...
			if (!userdata.user.s3.key_id.match(/^\w{20}$/)) {
				this.response.status = 400;
			...
			// check if s3 id is valid
			if (!userdata.user.s3.key_secret.match(/^[\W|\w]{40}$/)) {
				this.response.status = 400;
