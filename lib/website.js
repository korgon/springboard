...
website.prototype.commit = function(message) {
	...
	return new Promise(function(resolve, reject) {
		...
		try {
			git.exec({args: 'add -v sites/' + self.name, log: true, cwd: site_dir}, function (err) {
				if (err) return reject(err);
				try {
...
website.prototype.mergeit = function() {
	// promisified
	var self = this;
	return new Promise(function(resolve, reject) {
		self.pushit()
		.then(function() {
			git.exec({args: 'checkout master', log: true, cwd: site_dir}, function (err) {
				...
				git.pull('origin', 'master', {cwd: site_dir}, function (err) {
					if (err) return reject(err);
					git.merge('branchName', {cwd: site_dir}, function (err) {
						if (err) return reject(err);
